...
__all__ = ["app_user_session", "choice"]  # noqa
...
account = sa.Table(
    ...
    PrimaryKeyConstraint("id", name="app_user_account_pkey"),
)


login_attempt = sa.Table(
    "app_user_login_attempt",
    meta,
    Column("id", sa.Integer, primary_key=True),
    Column("user_id", sa.Integer, ForeignKey("app_user_account.id"), nullable=False),
    Column("creation_time", sa.DateTime, nullable=False),
...
session = sa.Table(
    ...
    Column("user_id", sa.Integer, ForeignKey("app_user_account.id"), nullable=True),
    ...
    Column("client_info", sa.String, nullable=False),
    sa.PrimaryKeyConstraint("id", name="app_user_session_pkey"),
)


class PublicMembershipStore(MembershipStore):
    """
