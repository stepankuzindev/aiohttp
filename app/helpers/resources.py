...
def resources(names, conf=None, development=False, cache_seed=None):
    """
    ...
    for name in names:
        if name not in sets:
            raise ValueError(
                "The set `{}` is not configured inside /configuration/scripts.js".format(
                    name
                )
            )
        if minification:
            a.append(
                '<script src="/scripts/{}{}?s={}"></script>'.format(
                    name, ".min.js", cache_seed
                )
            )
        elif bundling:
            a.append(
                '<script src="/scripts/{}{}?s={}"></script>'.format(
                    name, ".built.js", cache_seed
                )
            )
        else:
            files = sets[name]
...
def load_resources_config():
    ...
    try:
        ...
        contents = re.sub(";\s*$", "", contents)  # noqa
        data = json.loads(contents)
        return data
    except Exception as ex:
        print(
            "ERROR: while loading the scripts configuration for the resources helper."
        )
        print(str(ex))
        raise
