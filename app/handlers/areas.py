...
from .security.antiforgery import (
    issue_aft,
    validate_aft,
    InvalidAntiforgeryTokenException,
)
...
class Area:
    ...
    def __init__(self, name, membership_provider=None, fallback_url=None):
        ...
        if not configuration.areas:
            raise RuntimeError(
                "The configuration section 'areas' is not configured in the application configuration."
            )
        try:
            area_config = configuration.areas[name]
        except KeyError:
            raise RuntimeError(
                "The '{}' area is not configured in the application configuration.".format(
                    name
                )
            )
        ...
        if membership_provider:
            ...
            for property_name in {"session_cookie_name", "encryption_key"}:
                if not area_config.get(property_name):
                    raise RuntimeError(
                        "The '{}' area does not define its '{}'.".format(
                            name, property_name
                        )
                    )
            ...
            async def wrapped(request):
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        "Use the {0}.auth decorator after a '{0}' decorator".format(
                            area_name
                        )
                    )
    ...
    async def before_request(self, request):
        ...
        request[request_context_key] = {
            ...
            "_": partial(
                get_text, request.culture
            ),  # function for localization of strings inside template
        }
