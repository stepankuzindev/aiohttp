...
def get_text(language, key, default=None):
    ...
    for part in parts:
        if part not in v:
            return (
                "Missing translation for `{}`".format(key)
                if default is None
                else default
            )
        v = v[part]
...
def get_best_culture(request, area):
    ...
    if not area:
        raise RuntimeError(
            "The area '{}' is not configured in the application configuration file.".format(
                area
            )
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Cannot determine the default culture for area {}."
            "Either configure an area default_culture or a global default_culture".format(
                area
            )
        )
