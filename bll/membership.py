...
class Principal:
    ...
    def __init__(self, _id, identity_data, session, authenticated):
        self.id = _id
        self.identity = (
            self.get_identity_type().from_dict(identity_data)
            if identity_data is not None
            else AnonymousIdentity()
        )
...
class Identity:
    ...
    def __init__(self, _id, email, culture, roles, opts, created_timestamp):
        self.id = _id
        self.email = email
...
class MembershipProvider:
    ...
    def validate_store(store):
        ...
        for name in req:
            if not hasattr(store, name):
                raise Exception(
                    "The given store does not implement `" + name + "` member"
                )
    ...
    def validate_userkey(self, userkey):
        if userkey is None or re.match("^\s*$", userkey):  # noqa
            return False
        return True
    ...
    def create_account(self, userkey, password, data=None, roles=None, lang="en"):
        ...
        account_data = self.store.create_account(
            userkey, hashedpassword, salt, data, roles
        )
    ...
    def confirm_account(self, account_id, confirmation_token):
        ...
        self.store.update_account_by_id(
            account_id, {"confirmed": True}, unset_data={"confirmation_token": ""}
        )
        return True
    ...
    async def initialize_anonymous_session(self, client_ip, client_data):
        ...
        user_id = None
        session_data = await self.store.create_session(
            user_id, expiration, client_ip, client_data
        )
        session = Session.from_dict(session_data)
        principal_type = self.principal_type
        return AuthenticationResult(principal_type(None, None, session, False), session)
    ...
    def get_new_expiration(self, remember=None):
        ...
        now = datetime.datetime.utcnow()
        ms = (
            self.options.long_time_expiration
            if remember
            else self.options.short_time_expiration
        )
